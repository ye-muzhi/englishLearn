...
![EnglishLearn 主界面：沉浸式双语播放器与项目库](docs/assets/home-player.png)
...
- **仅供个人学习与研究使用**：本项目为非商业开源项目，不用于任何商业活动，也不提供任何形式的商业服务、付费功能或授权。
- **关于视频下载**：本项目集成 yt-dlp 以支持将视频下载到本地进行离线学习。下载功能仅供使用者获取其有权访问的内容用于个人学习，使用者应自行了解并遵守所在地区法律法规以及相关平台的服务条款；因下载、传播或再发布受版权保护内容而产生的一切责任，由使用者自行承担，与本项目及作者无关。
- **本仓库不含任何第三方音视频内容**：项目不存储、不分发任何视频、音频或字幕文件，仓库中亦不包含此类文件。
